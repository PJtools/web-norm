...
**用更合理的方式写 JavaScript**
  > 优先遵从 `ECMAScript 6` 开发编码规范，这是未来；不支持新特性的再退而其次遵从 `ECMAScript 5` 开发编码规范。
...
## 引用
  ...
  - 如果你一定需要可变动的引用，`ES6` 须使用 `let` 代替 `var`（`ES5` 没有 `let` 则只能使用 `var`）。
...
## 函数
  ...
  - 使用函数声明代替函数表达式。
    ...
    > 为什么？因为函数声明是可命名的，所以他们在调用栈中更容易被识别。此外，函数声明会把整个函数提升，而函数表达式只会把函数的引用变量名提升。这条规则使得[箭头函数](#arrow-functions)可以取代函数表达式。
...
## Iterators(迭代器) and Generators(生成器) [ES6]
  ...
  - 在理解不透彻或需要编译到ES5时，尽量少使用 Generators。
...
## 变量
  ...
  - 一直使用 `const` 来声明变量，如果不这样做就会产生全局变量。我们需要避免全局命名空间的污染。
    ...
    > 为什么？全局，global 亦有全球的意思。地球队长的责任是保卫地球环境，所以他警告我们不要造成「全球」污染。
...
## 注释
  ...
  - 文件顶部必须包含文件注释，用 `@file` 和 `@author` 标识文件说明。
  ...
  - 函数/方法注释必须包含函数说明，有参数和返回值时必须使用注释标识。参数和返回值注释必须包含类型信息，且不允许省略参数的说明。当函数是内部函数，外部不可访问时，可以使用 `@inner` 标识。
...
## 命名规则
  ...
  - 不要使用下划线 `_` 结尾或开头来命名属性和方法（除 `ES5` 中定义对象的 `私有变量` 可使用 `_` 开头）。
...
## 事件
  ...
  - 当给事件附加数据时（无论是 DOM 事件还是私有事件），传入一个哈希而不是原始值。这样可以让后面的开发人员增加更多数据到事件数据而无需找出并更新事件的每一个处理器。
...
## ECMAScript 5 浏览器的兼容性
  ...
  - 参考 Kangax 的 ES5 [兼容性](http://kangax.github.com/es5-compat-table/)。
