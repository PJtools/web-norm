...
## 引用
  ...
  - [2.2](#2.2) <a name='2.2'></a> 如果你一定需要可变动的引用，`ES6` 须使用 `let` 代替 `var`（`ES5` 没有 `let` 则只能使用 `var`）。
...
## 对象
  ...
  - [3.2](#3.2) <a name='3.2'></a> 如果你的代码在浏览器环境下执行，别使用 [保留字](http://es5.github.io/#x7.6.1) 作为键值。这样的话在 IE8 不会运行。但在 `ES6` 模块和服务器端中使用没有问题。
...
## Strings
  ...
  - [6.3](#6.3) <a name='6.3'></a> 注：过度使用字串连接符号可能会对性能造成影响。
...
## Iterators(迭代器) and Generators(生成器) [ES6]
  ...
  - [11.2](#11.2) <a name='11.2'></a> 在理解不透彻或需要编译到ES5时，尽量少使用 Generators。
...
## 逗号
  ...
  - [19.2](#19.2) <a name='19.2'></a> 增加结尾的逗号: **需要**。 [`ES6` 需要 `babel` 转义]
...
## 事件
  ...
  - [24.1](#24.1) <a name='24.1'></a> 当给事件附加数据时（无论是 DOM 事件还是私有事件），传入一个哈希而不是原始值。这样可以让后面的开发人员增加更多数据到事件数据而无需找出并更新事件的每一个处理器。
